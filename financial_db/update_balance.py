...
def calculate_change(balance, records, currency):
    ...
    # fillna for new account
    merged['observation_date'].fillna(method='ffill', inplace=True)
    merged['observation_date'].fillna(method='bfill', inplace=True)
    merged['account'] = merged['bank_acct']
    merged['balance'].fillna(0, inplace=True)
